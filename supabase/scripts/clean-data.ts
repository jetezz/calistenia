...
// Force load .env.test to ensure we rely on test credentials
config({ path: path.resolve(process.cwd(), ".env.test") });

process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0";

// In .env.test we use VITE_ prefixes now
const supabaseUrl = process.env.VITE_SUPABASE_URL;
const supabaseAnonKey = process.env.VITE_SUPABASE_ANON_KEY;
// In .env.test credentials are just ADMIN_EMAIL/PASSWORD without VITE_ prefix (check file content again to be sure)
// Let's check the file content from Step 36
// .env.test has ADMIN_EMAIL and ADMIN_PASSWORD
// BUT clean-data.ts previously used VITE_ADMIN_EMAIL.
// So we need to match what's in .env.test exactly.
...
async function cleanAllData() {
  ...
  if (!adminEmail || !adminPassword) {
    throw new Error(
      "Admin credentials not found. Please set ADMIN_EMAIL and ADMIN_PASSWORD in .env file",
    );
  }
