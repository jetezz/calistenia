import { Smartphone, Mail, Phone, Landmark, Wallet, AlertCircle, Clock, MapPin } from 'lucide-react'
...
export function PaymentInfoPage() {
  const { methods, isLoading } = usePaymentMethod()
        ...
        {methods.length === 0 ? (
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <h3 className="text-lg font-semibold mb-2">No hay métodos de pago disponibles</h3>
              <p className="text-muted-foreground">
                ...
                <Card key={method.id} className={`border-${color}-200`}>
                  <CardHeader>
                    <CardTitle className={`flex items-center gap-2 text-${color}-700`}>
                      ...
                      {method.name}
                      {isRecommended && (
                        <Badge className={`bg-${color}-100 text-${color}-700`}>Recomendado</Badge>
                      )}
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    {method.contact_phone && (
                      <div className="text-center">
                        <div className={`text-2xl font-mono font-bold text-${color}-600 bg-${color}-50 p-4 rounded-lg`}>
                          {method.contact_phone}
                        </div>
                        <Button
                          variant="outline"
                          size="sm"
                          className="mt-2"
                          onClick={() => copyToClipboard(method.contact_phone!.replace(/\s/g, ''), `Número de ${method.name}`)}
                        >
                          Copiar número
                        </Button>
                      </div>
                    ...
                    {method.contact_email && (
                      <div className="text-center">
                        <div className={`text-lg font-mono text-${color}-600 bg-${color}-50 p-4 rounded-lg break-all`}>
                          {method.contact_email}
                        </div>
                        <Button
                          variant="outline"
                          size="sm"
                          className="mt-2"
                          onClick={() => copyToClipboard(method.contact_email!, `Email de ${method.name}`)}
                        >
                          Copiar email
                        </Button>
                      </div>
                    ...
                    {method.bank_account && (
                      <div className="text-center">
                        <div className={`text-sm font-mono text-${color}-600 bg-${color}-50 p-4 rounded-lg break-all`}>
                          {method.bank_account}
                        </div>
                        <Button
                          variant="outline"
                          size="sm"
                          className="mt-2"
                          onClick={() => copyToClipboard(method.bank_account!.replace(/\s/g, ''), 'Cuenta bancaria')}
                        >
                          Copiar cuenta
                        </Button>
                      </div>
                    ...
                    <div className={`bg-${color}-50 p-3 rounded-lg`}>
                      <div className="flex gap-2 text-sm">
                        <AlertCircle className={`size-4 text-${color}-600 shrink-0 mt-0.5`} />
                        <div className={`text-${color}-700`}>
                          <p className="font-medium">Método {method.type === 'cash' ? 'presencial' : 'seguro'}</p>
                          <p>{method.type === 'cash' ? 'Paga en el centro' : 'Pago protegido y verificado'}</p>
                        </div>
                      </div>
                    </div>
                  </CardContent>
