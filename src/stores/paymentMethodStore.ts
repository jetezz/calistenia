...
interface PaymentMethodStore {
  ...
  updateMethod: (id: string, updates: PaymentMethodUpdate) => Promise<PaymentMethod>
...
export const usePaymentMethodStore = create<PaymentMethodStore>((set, get) => ({
  ...
  fetchMethods: async () => {
    ...
    try {
      ...
      set({ methods, isLoading: false })
    } catch (error) {
      set({ 
        error: error instanceof Error ? error.message : 'Error fetching payment methods',
        isLoading: false 
      })
    }
  },
  ...
  fetchActiveMethods: async () => {
    ...
    try {
      ...
      set({ 
        error: error instanceof Error ? error.message : 'Error fetching active methods',
        isLoading: false 
      })
    }
  },
  ...
  createMethod: async (paymentMethod) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Error creating payment method'
  ...
  updateMethod: async (id, updates) => {
    ...
    try {
      ...
      set({ 
        methods: get().methods.map(method => method.id === id ? updatedMethod : method),
        isLoading: false 
      })
