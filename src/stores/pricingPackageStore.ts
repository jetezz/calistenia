...
interface PricingPackageStore {
  ...
  createPackage: (pricingPackage: PricingPackageInsert) => Promise<PricingPackage>
  updatePackage: (id: string, updates: PricingPackageUpdate) => Promise<PricingPackage>
  deletePackage: (id: string) => Promise<void>
  toggleActive: (id: string, isActive: boolean) => Promise<void>
  setError: (error: string | null) => void
}

export const usePricingPackageStore = create<PricingPackageStore>((set, get) => ({
  packages: [],
  isLoading: false,
  error: null,
  ...
  fetchPackages: async () => {
    ...
    try {
      ...
      set({ 
        error: error instanceof Error ? error.message : 'Error fetching pricing packages',
        isLoading: false 
      })
    }
  },
  ...
  fetchActivePackages: async () => {
    ...
    try {
      ...
      set({ 
        error: error instanceof Error ? error.message : 'Error fetching active packages',
        isLoading: false 
      })
    }
  },
  ...
  createPackage: async (pricingPackage) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Error creating pricing package'
  ...
  updatePackage: async (id, updates) => {
    ...
    try {
      ...
      set({ 
        packages: get().packages.map(pkg => pkg.id === id ? updatedPackage : pkg),
        isLoading: false 
      })
      return updatedPackage
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Error updating pricing package'
      set({ error: errorMessage, isLoading: false })
      throw error
    }
  },
  ...
  deletePackage: async (id) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Error deleting pricing package'
      set({ error: errorMessage, isLoading: false })
      throw error
    }
  },
