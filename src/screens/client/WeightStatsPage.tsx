      ...
      {hasData && latestMetrics && (
        <Tabs defaultValue="overview" className="space-y-6">
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            {/* Metrics Overview */}
            <MetricsOverview metrics={latestMetrics} trends={trends} />

            {/* Gráficas especializadas */}
            <div className="space-y-6">
              <WeightCompositionChart data={weightCompositionData} />
              <BodyPercentagesChart data={bodyPercentagesData} />
            ...
            {latestStat?.notes && (
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">Notas de la Última Medición</CardTitle>
                </CardHeader>
                <CardContent>
                  <p className="text-sm text-muted-foreground">{latestStat.notes}</p>
                </CardContent>
              </Card>
            )}
          </TabsContent>
          ...
          <TabsContent value="history" className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <p className="text-sm text-muted-foreground">
                Total: {stats.length} {stats.length === 1 ? "medición" : "mediciones"}
              </p>
            </div>
                ...
                <Card key={stat.id}>
                  <CardHeader>
                    <CardTitle className="text-base">
                      {new Date(stat.recorded_at).toLocaleDateString("es-ES", {
                        day: "numeric",
                        month: "long",
                        year: "numeric",
                      })}
                    ...
                    <div className="grid gap-2 text-sm">
                      ...
                      {stat.body_fat_percentage && (
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{stat.body_fat_percentage}%</span>
                        </div>
                      )}
                      {stat.muscle_mass && (
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Músculo:</span>
                          <span className="font-medium">{stat.muscle_mass} kg</span>
                        </div>
                      )}
                      {stat.bone_mass && (
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Hueso:</span>
                          <span className="font-medium">{stat.bone_mass} kg</span>
                        </div>
                      )}
                      {stat.bmi && (
                        <div className="flex justify-between">
                      ...
                      {stat.total_body_water_percentage && (
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{stat.total_body_water_percentage}%</span>
                        </div>
                      )}
                      {stat.daily_calorie_intake && (
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Calorías:</span>
                          <span className="font-medium">{stat.daily_calorie_intake} kcal</span>
                        </div>
                      )}
                      {stat.metabolic_age && (
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Edad Metabólica:</span>
                          <span className="font-medium">{stat.metabolic_age} años</span>
                        </div>
                      )}
                      {stat.notes && (
                        <div className="mt-2 pt-2 border-t">
                          <p className="text-muted-foreground italic">{stat.notes}</p>
                        </div>
                      )}
                    </div>
                  </CardContent>
